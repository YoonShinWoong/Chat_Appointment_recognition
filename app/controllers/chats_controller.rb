  ...
  def create
    ...
    respond_to do |format|
      if @chat.save
        ...
        if @post.alarmstat.nil? || @post.alarmstat =="대기"
          ...
          # 약속 날짜 체크
          kdate = korean_date_check(@chat.body)
          wdate = wday_check(@chat.body)
          owdate = only_wday_check(@chat.body)
          date = date_check(@chat.body)
          var_date = nil
          if kdate then var_date = kdate 
          elsif wdate then var_date = wdate 
          elsif owdate then var_date = owdate 
          elsif date  then var_date = date 
          end

          if var_date
            @post.date = var_date
            @post.time_age = var_date
            @post.save
            notice_msg = "구체적인 약속 날짜가 이야기되고 있네요~ 아래와 같은 알람을 보내드릴 수 있어요."
          end

          # 약속 시간 체크
          time = time_check(@chat.body)
          if time
            @post.time = time
            @post.save
            notice_msg = "구체적인 약속 시간이 이야기되고 있네요~ 아래와 같은 알람을 보내드릴 수 있어요."
          end
        end
